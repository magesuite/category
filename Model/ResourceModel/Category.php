...
namespace MageSuite\Category\Model\ResourceModel;
...
class Category
{
    ...
    public function getProductCount(\Magento\Catalog\Model\Category $category) {
        $counts = $this->getProductCountInAllCategories();

        return $counts[$category->getId()] ?? 0;
    }

    /**
     * Internal method $category->getProductCount() returns only amount of categories in specific category without
     * taking subcategories into account, this method returns counts also with subcategories
